...
deploy ALL=(root) NOPASSWD: /bin/systemctl reload caddy, /usr/bin/systemctl reload caddy, /usr/bin/tee /etc/caddy/Caddyfile.d/*, /usr/bin/tee /etc/caddy/Caddyfile, /bin/rm /etc/caddy/Caddyfile.d/*, /usr/bin/install -d -o caddy -g caddy /var/www/*, /usr/bin/chown -R caddy\:caddy /var/www/*, /usr/bin/rsync -a --delete /opt/sepsisatlas/*, /bin/rm -rf /var/www/atlas-*, /usr/bin/cp /opt/sepsisatlas/*/deploy/Caddyfile /etc/caddy/Caddyfile
EOF
